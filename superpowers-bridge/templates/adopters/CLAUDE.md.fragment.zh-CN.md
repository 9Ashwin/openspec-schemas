...
本 repo 采用 [`superpowers-bridge`](https://github.com/JiangWay/openspec-schemas/tree/main/superpowers-bridge) 衔接 OpenSpec 与 Superpowers。集成规则(语言、artifact 路径、PRECHECK)以该 bridge README 为准;以下是给 Claude 的 routing 指引。

### 入口分流

| 你看到的触发 | 应该怎么做 |
|---|---|
| 用户以 narrative 开「设计讨论 / 脑力激荡」 | 先 verbal `superpowers:brainstorming`,**不**写到 `docs/superpowers/specs/`;对话收敛后依下方 5 条判准升级到 `/opsx:propose` |
...
详细见 [superpowers-bridge README §进入与离开的判断](https://github.com/JiangWay/openspec-schemas/blob/main/superpowers-bridge/README.zh-TW.md#进入与离开的判断entry--exit-gates)。
