...
> 把 [OpenSpec](https://github.com/Fission-AI/OpenSpec) 的 artifact 治理流程(**做什么**)与 [obra/superpowers](https://github.com/obra/superpowers) 的运行技能(**怎么做**)集成为单一工作流。额外提供 evidence-first 的 `retrospective` artifact,补上 Superpowers 没有的 retro 能力。
>
> 集成**完全发生在 prompt 层**——不修改 Superpowers 任何代码,不修改 OpenSpec CLI。Schema 版本:v1。
...
在你项目的根目录打开 Claude Code,把下面这段贴进去:
...
> `<locale>` 缺省 `zh-TW`(若你 CLAUDE.md 是繁中)或省略(英文)。Claude 会依你 CLAUDE.md 的既有语言判断。
...
| `CLAUDE.md`(项目根) | schema 目录附 `templates/adopters/CLAUDE.md.fragment.<locale>.md` 片段;升级流程会把你的 CLAUDE.md 跟此片段 diff 给你看,等你 ack 才插入或更新 | 需要 — 确认 diff,选择插入 / 取代 / 保留现有 |

> bridge 目录是 monolithic — 要嘛整包换新版,要嘛留旧版,**没有逐档 opt-in**。CLAUDE.md 是升级流程唯一会碰项目根目录的文件,而且永远等你 ack。
...
OpenSpec 管 **「做什么」**(artifact 生命周期:proposal / specs / tasks / verify 等)。Superpowers 管 **「怎么做」**(运行纪律:brainstorming、writing-plans、TDD、code review)。各自坚实,但实际开发中交替使用会出现三个结构性问题:
...
本 schema 的 instruction 只在 `/opsx:*` 指令启动 artifact 时才注入 prompt。如果你以 narrative 方式触发 Superpowers skill(例如直接对 Claude 说「我们讨论一下架构」),缺省行为会绕过本 schema —— brainstorming 仍会写到 `docs/superpowers/specs/`,让集成的 redirection 完全失效。
...
如果用户以 narrative(「我们来讨论架构」「脑力激荡一下」)触发了 `superpowers:brainstorming`,brainstorming 的产出**不可以**写到 `docs/superpowers/specs/` —— 那会绕过本 schema 的 output redirection,在 repo 里留下 orphan artifact。
...
| 添加 artifacts | — | brainstorm, plan, verify, retrospective |

### Lifecycle(apply 编排 + 时序注记)

上方 Artifact DAG 显示**文件存在**依赖。下面这张完整 lifecycle 加上 apply phase 的顺序步骤,以及 graph 边与实际产出时序的**错位**。
...
> - `retrospective.md` 声明 `requires: verify`,并依 Step 4 在 PR 打开**之前**产出 —— PR diff 才会包含完整 archived cycle(所有 artifact 完成、spec 已 sync、change folder 在 `archive/`)。
...
> **不支持 `executing-plans` fallback**。本 schema 是 opinionated 的:要求 subagent-capable 平台(Claude Code、Codex 等)。替代 executor `superpowers:executing-plans` 并**不会** transitively 触发 TDD 或 code-review(已对 [SKILL.md](https://github.com/obra/superpowers/blob/main/skills/executing-plans/SKILL.md) 做事实查核 —— body 完全没提到 TDD 或 code-review,Integration 段也未列出 `test-driven-development` 与 `requesting-code-review`)。退到 2b 等于静默降级 Superpowers 的内核价值。若你的平台没有 subagent 支持,改用 OpenSpec 内置的 `spec-driven` schema。

### Output redirection(产出重导)

Superpowers skill 有缺省输出路径(例如 brainstorming 写到 `docs/superpowers/specs/`)。本 schema 的 artifact instruction **覆写**这个行为,通过 prompt 上下文注入,把产出重导到 change 目录:

- brainstorming → `openspec/changes/<name>/brainstorm.md`(可选 `design.md`)
- writing-plans → `openspec/changes/<name>/plan.md`

纯粹通过 invocation-time 上下文注入实现,不修改 skill 源码。
...
# 或修改项目缺省(openspec/config.yaml: schema: spec-driven)
```
...
skill 缺失 → STOP 并通知用户,不静默 fallback,本 schema 内也没有 manual mode。建议用户在那个 change 改用 OpenSpec 内置的 `spec-driven` schema,或安装缺失的 skill 后重来。

> 本 schema 的 v0 版本曾在这里放「自动 commit change artifacts 到当前分支」逻辑,在 [PR #970 review](https://github.com/Fission-AI/OpenSpec/pull/970) 后移除:处理未追踪的 change 目录是 worktree skill 的责任,schema 不该主动改写用户的 git history。

#### 1. Workspace — `superpowers:using-git-worktrees`

创建 `.worktrees/<change-name>/`、切到新 branch、跑项目 setup、确认 test baseline 干净。
...
本 schema **不支持** `superpowers:executing-plans` 作为 fallback。理由见下方「六个值得记住的设计触点」段。
...
| 新 change(交互式) | `/opsx:new <name> --schema superpowers-bridge` 接着多次 `/opsx:continue` |
...
| 用内置(跳过 brainstorm) | `/opsx:new <name> --schema spec-driven` |
...
### 2. Schema-level vs prompt-level 集成

集成**完全**发生在 `instruction:` 字段(纯 prompt)。Superpowers 升版某个 skill 的行为时,本 schema 不用改。只有 skill 被改名或移除时才要 touch `schema.yaml`。
...
本 schema 要求 subagent-capable 平台(Claude Code、Codex 等)。替代 executor `superpowers:executing-plans` **不会** transitively 触发 TDD 或 code-review(已对其 [SKILL.md](https://github.com/obra/superpowers/blob/main/skills/executing-plans/SKILL.md) 做事实查核 —— body 完全没提及这两者,Integration 段也未列出 `test-driven-development` 与 `requesting-code-review`)。退到 2b 等于静默丢掉 Superpowers 带给集成的内核价值。我们选择在 Step 0 fail loud,并指引用户改用内置的 `spec-driven` schema。
...
- ⚠️ **不会自动侦测** —— Superpowers skill 的行为变动(skill 改名、改写 prose 而影响 PRECHECK 语意、传递依赖变动)。version-check workflow 侦测到新版时开 issue,提醒人类去读 release notes。
...
Brainstorming 是多轮交互对话,需要用户参与。把它做为第一个 artifact(而非 schema-level hook)有两个好处:
...
`tasks.md` 是粗粒度 checkbox(「添加 PdfServiceTest」);`plan.md` 是 micro-step(「建测试骨架 → 写 downloadPdf 测试 → 跑 → commit」)。两者粒度与用途不同:

- `tasks.md` → 追踪整体进度(apply phase 的 `tracks` 字段解析 checkbox)
...
- **`brainstorm` / `plan` artifact**:用户可明确 opt-in 改成手动撰写(PRECHECK 会 STOP 并通知;手动模式需要用户明确选择,不会静默降级)
- **`apply` phase**:本 schema 没有 manual fallback。Step 0 PRECHECK 缺任何必要 skill 就 STOP,建议改用 OpenSpec 内置的 `spec-driven` schema 跑那个 change。理由见上面「设计触点 #4」—— `executing-plans` 不会 transitively 触发 TDD 与 code-review,降级的 apply 等于违背 schema 的目的
